...
func TestCheckEngineFailsWhenConfiguredSourceIsMissingFromLockfile(t *testing.T) {
	eng := &CheckEngine{ToolMap: target.NewToolMap(nil), ProjectRoot: t.TempDir()}
	cfg := config.Config{
		Version: 1,
		Sources: []config.Source{{Name: "src", Type: "local"}},
	}

	result, err := eng.Check(context.Background(), lock.Lockfile{Version: 1}, cfg)
	if err != nil {
		t.Fatalf("Check() error = %v", err)
	}
	if result.Clean {
		t.Fatal("Check() reported clean with a configured source missing from the lockfile")
	}
	if len(result.Errors) != 1 || !strings.Contains(result.Errors[0].Error(), "missing from the lockfile") {
		t.Fatalf("Check() errors = %v, want missing lockfile entry", result.Errors)
	}
}

func TestCheckEngineUsesTransformedAndOverriddenContent(t *testing.T) {
	projectRoot := t.TempDir()
	...
	originalHash := cache.ComputeHash(original)
	if putErr := contentCache.Put(originalHash, original); putErr != nil {
		t.Fatal(putErr)
	}
	if writeErr := os.WriteFile(filepath.Join(projectRoot, "footer.md"), []byte("Generated by Leorna"), 0644); writeErr != nil {
		t.Fatal(writeErr)
	}
	if mkdirErr := os.MkdirAll(filepath.Join(projectRoot, ".out"), 0755); mkdirErr != nil {
		t.Fatal(mkdirErr)
	}
	expected := []byte("Hello World!\nGenerated by Leorna")
	if writeErr := os.WriteFile(filepath.Join(projectRoot, ".out", "greeting.md"), expected, 0644); writeErr != nil {
		t.Fatal(writeErr)
	}
	...
	cfg := config.Config{
		...
		Sources:    []config.Source{{Name: "src", Type: "local"}},
		Targets:    []config.Target{{Source: "src", Destination: ".out/"}},
		Transforms: []config.Transform{{Source: "src", Type: "template"}},
		Overrides:  []config.Override{{Target: "greeting.md", Strategy: "append", File: "footer.md"}},
	}
	lf := lock.Lockfile{Version: 1, Sources: []lock.LockedSource{{
		Name: "src", Type: "local", Status: "ok",
		Resolved: lock.ResolvedState{Files: map[string]lock.FileHash{
			"greeting.md": {SHA256: originalHash},
		}},
	}}}
	...
	if !result.Clean {
		t.Fatalf("Check() = %+v, want transformed target to be clean", result)
	}
}
