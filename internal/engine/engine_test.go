...
func TestCheckEngineUsesTransformedAndOverriddenContent(t *testing.T) {
	...
	if err := contentCache.Put(originalHash, original); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(projectRoot, "footer.md"), []byte("Generated by Leorna"), 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Join(projectRoot, ".out"), 0755); err != nil {
		t.Fatal(err)
	}
	expected := []byte("Hello World!\nGenerated by Leorna")
	if err := os.WriteFile(filepath.Join(projectRoot, ".out", "greeting.md"), expected, 0644); err != nil {
		t.Fatal(err)
	}
